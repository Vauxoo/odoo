...
class SaleOrderLine(models.Model):
    ...
    task_id = fields.Many2one(
        ...
        index=True, copy=False, export_string_translation=False)
    reached_milestones_ids = fields.One2many('project.milestone', 'sale_line_id', domain=[('is_reached', '=', True)], export_string_translation=False)
    ...
    def create(self, vals_list):
        ...
        for line in confirmed_lines - has_task_lines:
            if line.task_id:
                msg_body = self.env._("Task Created (%(name)s): %(link)s", name=line.product_id.name, link=line.task_id._get_html_link())
    ...
    def _timesheet_create_task(self, project):
        ...
        task_msg = self.env._("This task has been created from: %(order_link)s (%(product_name)s)",

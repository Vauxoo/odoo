    ...
    def write(self, vals):
        ...
        if 'active' in vals and not vals['active']:
            ...
            if will_be_all_lang_survey_sudo := surveys_sudo.filtered(lambda survey: survey.lang_ids <= self):
                if len(self) > 1:
                    error = self.env._("Cannot deactivate languages currently used by survey(s) only supporting those languages.")
                else:
                    error = self.env._("Cannot deactivate a language currently used by survey(s) only supporting that language.")
                if self.env['survey.survey'].search(
                        [('id', 'in', will_be_all_lang_survey_sudo.ids)]) == will_be_all_lang_survey_sudo:
                    error += '\n'
                    error += self.env._("Survey(s): %(surveys_list)s",

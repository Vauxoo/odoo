...
class ResPartnerBank(models.Model):
    ...
    def _check_mm_proxy(self):
        for bank in self.filtered(lambda b: b.country_code == 'MM'):
            if bank.proxy_type not in ('merchant_id', 'none', False):
                raise ValidationError(self.env._("The proxy type of an MMQR code must be a Merchant ID."))
            if bank.proxy_type == 'merchant_id' and not (bank.proxy_value or '').isdigit():
                raise ValidationError(self.env._("The Merchant ID of an MMQR code must only contain digits."))
            ...
            if bank.l10n_mm_terminal_id and not re.fullmatch(r'\d{1,25}', bank.l10n_mm_terminal_id):
                raise ValidationError(self.env._("The Terminal ID of an MMQR code must be at most 25 digits long."))
    ...
    def _compute_country_proxy_keys(self):
        ...
        return super(ResPartnerBank, self - bank_mm)._compute_country_proxy_keys()

    @api.depends('country_code')
    def _compute_display_qr_setting(self):
        bank_mm = self.filtered(lambda b: b.country_code == 'MM')
        bank_mm.display_qr_setting = True
        return super(ResPartnerBank, self - bank_mm)._compute_display_qr_setting()
    ...
    def _get_error_messages_for_qr(self, qr_method, debtor_partner, currency):
        if qr_method == 'emv_qr' and self.country_code == 'MM':
            if currency.name != 'MMK':
                return self.env._("Can't generate an MMQR code with a currency other than MMK.")
            return None
    ...
    def _check_for_qr_code_errors(self, qr_method, amount, currency, debtor_partner, free_communication, structured_communication):
        if qr_method == 'emv_qr' and self.country_code == 'MM':
            if self.proxy_type != 'merchant_id':
                return self.env._("The proxy type of an MMQR code must be a Merchant ID.")
            if not self.l10n_mm_merchant_name:
                return self.env._("Missing Merchant Name (Myanmar Unicode).")

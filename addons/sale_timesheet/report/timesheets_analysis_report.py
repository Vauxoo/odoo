...
from ..models.account_analytic_line import TIMESHEET_BILLABLE_TYPES
...
class TimesheetsAnalysisReport(models.Model):
    ...
    billable_type = fields.Selection(BILLABLE_TYPES + TIMESHEET_BILLABLE_TYPES, readonly=True)
    reinvoice_move_id = fields.Many2one("account.move", string="Invoice", readonly=True, help="Invoice created from the timesheet")
    timesheet_revenues = fields.Monetary(currency_field="currency_id", readonly=True, help="Number of hours spent multiplied by the unit price per hour/day.")
    margin = fields.Monetary(currency_field="currency_id", readonly=True, help="Timesheets revenues minus the costs")
    billable_time = fields.Float(readonly=True, help="Number of hours/days linked to a SOL.")

...
class AccountMove(models.Model):
    ...
    def _l10n_ke_validate_move(self):
        ...
        for move in self:
            ...
            if move.country_code != 'KE':
                move_errors.append(self.env._("This invoice is not a Kenyan invoice and therefore can not be sent to the device."))

            if move.company_id.currency_id != self.env.ref('base.KES'):
                move_errors.append(self.env._("This invoice's company currency is not in Kenyan Shillings, conversion to KES is not possible."))

            if move.state != 'posted':
                move_errors.append(self.env._("This invoice/credit note has not been posted. Please confirm it to continue."))

            if move.move_type not in ('out_refund', 'out_invoice'):
                move_errors.append(self.env._("The document being sent should be an invoice or credit note."))

            if any([move.l10n_ke_cu_invoice_number, move.l10n_ke_cu_serial_number, move.l10n_ke_cu_qrcode, move.l10n_ke_cu_datetime]):
                move_errors.append(self.env._("The document already has details related to the fiscal device. Please make sure that the invoice has not already been sent."))
            ...
            if move.move_type == 'out_refund' and not move.reversed_entry_id.l10n_ke_cu_invoice_number:
                move_errors.append(self.env._("This credit note must reference the previous invoice, and this previous invoice must have already been submitted."))

            for line in self.invoice_line_ids.filtered(lambda l: l.display_type == 'product'):
                vat_taxes = line.tax_ids.filtered(lambda tax: tax.amount in (16, 8, 0))
                if not vat_taxes or len(vat_taxes) > 1:
                    move_errors.append(self.env._("On line %s, you must select one and only one VAT tax.", line.name))
                else:
                    if vat_taxes[0].amount == 0 and not line.tax_ids[0].l10n_ke_item_code_id:
                        move_errors.append(self.env._("On line %s, a tax with a KRA item code must be selected, since the tax is 0%% or exempt.", line.name))
    ...
    def l10n_ke_action_cu_post(self):
        ...
        if self.company_id.l10n_ke_oscu_is_active:
            raise UserError(
                self.env._('An OSCU has been initialized for this company. Please send the e-invoice via Send and Print -> Send to eTIMS instead.')
            )
        ...
        if errors:
            ...
            for move, error_list in errors:
                error_list = '\n'.join(error_list)
                error_msg += self.env._("Invalid invoice configuration on %(invoice)s:\n%(error_list)s\n\n", invoice=move, error_list=error_list)

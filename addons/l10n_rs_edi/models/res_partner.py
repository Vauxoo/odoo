    ...
    def _check_l10n_rs_edi_public_funds(self):
        for record in self:
            if record.l10n_rs_edi_public_funds and \
                ...
                raise ValidationError(self.env._('Public Funds ID(JBKJS) must be exactly five digits'))
    ...
    def _check_l10n_rs_edi_registration_number(self):
        for record in self:
            if record.l10n_rs_edi_registration_number and \
                ...
                raise ValidationError(self.env._('Customer identification number should be 8 or 13 digits'))

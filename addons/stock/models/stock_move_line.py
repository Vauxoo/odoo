...
class StockMoveLine(models.Model):
    ...
    company_id = fields.Many2one('res.company', readonly=True, required=True, index=True)
    product_id = fields.Many2one('product.product', ondelete="cascade", check_company=True, domain="[('type', '!=', 'service')]", index=True)
    ...
    quantity = fields.Float(
        digits='Product Unit', copy=False, store=True,
    ...
    picked = fields.Boolean(compute='_compute_picked', store=True, readonly=False, copy=False)
    ...
    package_history_id = fields.Many2one('stock.package.history', index='btree_not_null')

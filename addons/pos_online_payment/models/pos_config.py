        ...
        for config in self:
            ...
            for pm in config.payment_method_ids:
                if pm.type == 'online':
                    opm_amount += 1
                    if opm_amount > 1:
                        raise ValidationError(self.env._("A POS config cannot have more than one online payment method."))
                    if not pm._get_online_payment_providers(config.id, error_if_invalid=True):
                        raise ValidationError(self.env._("To use an online payment method in a POS config, it must have at least one published payment provider supporting the currency of that POS config."))

    def _get_cashier_online_payment_method(self):
        self.ensure_one()
        return self.payment_method_ids.filtered(lambda pm: pm.type == 'online')[:1]

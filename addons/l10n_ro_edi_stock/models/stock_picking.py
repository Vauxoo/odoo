...
from .l10n_ro_edi_stock_document import DOCUMENT_STATES
from .etransport_api import ETransportAPI
...
class Picking(models.Model):
    ...
    def _l10n_ro_edi_stock_validate_carrier(self):
        for picking in self.filtered(self._l10n_ro_edi_stock_validate_carrier_filter):
            ...
            if not picking.carrier_id:
                raise UserError(self.env._("The picking %(picking_name)s is missing a delivery carrier.", picking_name=picking.name))

            # validate carrier partner
            if not picking.carrier_id.l10n_ro_edi_stock_partner_id:
                raise UserError(self.env._("The delivery carrier of %(picking_name)s is missing the partner field value.", picking_name=picking.name))
    ...
    def _l10n_ro_edi_stock_validate_data(self, data: dict):
        ...
        if len(missing_carrier_partner_fields) == 1:
            errors.append(self.env._("The delivery carrier partner is missing the %(field_name)s field.", field_name=missing_carrier_partner_fields[0]))
        elif len(missing_carrier_partner_fields) > 1:
            errors.append(self.env._("The delivery carrier partner is missing following fields: %(field_names)s", field_names=', '.join(missing_carrier_partner_fields)))
        ...
        if 'intrastat_code_id' in self.env['product.product']._fields and data['l10n_ro_edi_stock_operation_type'] not in ('60', '70'):
            ...
            if product_without_code_names:
                if len(product_without_code_names) == 1:
                    ...
                    errors.append(self.env._("Product %(name)s is missing the intrastat code value.", name=product_name))
                else:
                    errors.append(self.env._("Products %(names)s are missing the intrastat code value.", names=", ".join(product_without_code_names)))

        # Location types
        if not data['l10n_ro_edi_stock_start_loc_type']:
            if not data['l10n_ro_edi_stock_end_loc_type']:
                errors.append(self.env._("Both 'End' and 'Start Location Type' are missing"))
            else:
        ...
        for location in ('start', 'end'):
            ...
            loc_group = self.env._("'Start Location'") if location == 'start' else self.env._("'End Location'")

            if loc_value == 'bcp' and not data[f'l10n_ro_edi_stock_{location}_bcp']:
                errors.append(self.env._("The border crossing point is missing under %(location_group)s", location_group=loc_group))
            elif loc_value == 'customs' and not data[f'l10n_ro_edi_stock_{location}_customs_office']:
                errors.append(self.env._("The customs office is missing under %(location_group)s", location_group=loc_group))
                ...
                match data['picking_type_id'].code:
                    ...
                    case _other:
                        errors.append(self.env._("Invalid picking type %(type_code)s", type_code=_other))
                        continue

                if partner.country_id != country_ro:
                    errors.append(self.env._("Warehouse of %(location_group)s should be in Romania", location_group=loc_group))
                ...
                if len(missing_field_names) == 1:
                    errors.append(self.env._("%(location_group)s is missing the %(field_name)s field.", location_group=loc_group, field_name=missing_field_names[0]))
                elif len(missing_field_names) > 1:
                    errors.append(self.env._("%(location_group)s is missing following fields: %(field_names)s", location_group=loc_group, field_names=missing_field_names))

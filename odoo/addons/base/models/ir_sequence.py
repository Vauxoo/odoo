...
class IrSequence(models.Model):
    ...
    @api.onchange('prefix', 'suffix')
    def _onchange_prefix_suffix_ascii(self):
        for field_name in ('prefix', 'suffix'):
            value = self[field_name] or ''
            if value and not value.isascii():
                return {
                    'warning': {
                        'title': self.env._("Non-ASCII characters in %s", field_name),
                        'message': self.env._(
                            "The %(field)s %(value)s contains non-ASCII characters. "
                            "Names generated by this sequence may be printed as Code128 "
                            "barcodes in reports, which only support ASCII characters.",
